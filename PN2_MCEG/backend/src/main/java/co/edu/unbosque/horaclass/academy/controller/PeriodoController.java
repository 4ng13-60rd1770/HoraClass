...
public class PeriodoController {
    ...
    @PostMapping
    public ResponseEntity<PeriodoResponseDto> crear(@RequestBody PeriodoRequestDto request) {
        return new ResponseEntity<>(service.crear(request), HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<PeriodoResponseDto> actualizar(@PathVariable int id,
            @RequestBody PeriodoRequestDto request) {
        return ResponseEntity.ok(service.actualizar(id, request));
    }

    @GetMapping
    public ResponseEntity<List<PeriodoResponseDto>> listarTodos() {
        return ResponseEntity.ok(service.listarTodos());
    }

    @GetMapping("/{id}")
    public ResponseEntity<PeriodoResponseDto> obtenerPorId(@PathVariable int id) {
